...
export const saveUserStatsAfterCreate = functions.auth.user().onCreate(event => {
  if (!isUndefined(event.data.displayName)) {
...
export const deleteUserStatsAfterDeletingUser = functions.auth.user().onDelete(event => {
  return admin.database().ref('rankings').child(event.data.uid).remove;
});

export const checkBoardForWinAfterMove = functions.database.ref('/games/{gameId}/board').onUpdate(event => {
...
export const saveStatsBeforeQuittingGame = functions.database.ref('/games/{gameId}/status').onUpdate(event => {
  return event.data.ref.parent.once('value').then(snap => {
    const game = snap.val();
...
function setStatsAfterWin(game: any, id: string) {
...
function setStatsBeforeQuit(playerNumber: number, game: any, id: string) {
...
function updateStats(game: any, id: string, winnerStats: any, winnerUid: string, opponentStats: any, opponentUid: string) {
...
function updateStatsBeforeQuit(playerNumber: any, game: any, id: string, winnerStats: any, winnerUid: string, opponentStats: any, opponentUid: string) {
  return updatePlayerStats(winnerStats, winnerUid)
    .then(() => {
      return updatePlayerStats(opponentStats, opponentUid)
        .then(() => {
...
function updatePlayerStats(playerStats: any, playerUid: string) {
...
function updateStatsInGame(game: any, id: string, winnerStats, opponentStats) {
  if (game.turn === 1) {
...
function updateStatsInGameAfterQuit(playerNumber: number, game: any, id: string, winnerStats, opponentStats) {
  if (playerNumber === 2) {
    return updatePlayerStatsInGame(game, id, winnerStats, opponentStats)
  } else {
    return updatePlayerStatsInGame(game, id, opponentStats, winnerStats);
...
function updatePlayerStatsInGame(game: any, id: string, player1Stats: any, player2Stats: any) {
  return admin.database().ref('/games/' + id).update({
    player1: {
